...
let server = null;
let startedMinions = false;
let stoppingMinions = true;
...
class Module {
    ...
    async handleMessage(buffer) {
        ...
        switch (opcode) {
            ...
            case 2:
                ...
                break;
            case 3:
                this.handleSplitMinions();
                break;
            case 4:
                this.handleFeedMinions();
                break;
            case 5:
    ...
    startMinions() {
        if (!startedMinions) {
            stoppingMinions = false;
            for (let i = 0; i < config.minionsAmount; i++) this.minions.push(new NELBOTS(i, this.ws));
            this.minionInt = setInterval(() => { this.updateAliveMinions(); }, 200);
            Logger.info(`User Starting minions`);
        }
    }
    stopMinions() {
        if (startedMinions && !stoppingMinions) {
            clearInterval(this.minionInt);
            this.minions.forEach(minion => minion.stop());
            this.minions = [];
            this.minionInt = null;
            server = null;
            startedMinions = false;
            stoppingMinions = true;
            this.ws.send(Buffer.from([1])); 
            Logger.warn(`Minions Stopped!`);
        }
    }
    handleSplitMinions() {
        for (const minion of this.minions) {
            if (minion.ws && !stoppingMinions) {
                minion.split();
                let ejectCount = 3;
    ...
    handleFeedMinions() {
        for (const minion of this.minions) if (minion.ws && !stoppingMinions) minion.eject();
    }
    handleMouse(reader) {
        const mouseX = reader.readInt32();
        const mouseY = reader.readInt32();
        for (const minion of this.minions) minion.playerPos = { x: mouseX, y: mouseY };
    }
    handleSendCaptcha(reader) {
        const id = reader.readUint8();
        const token = reader.readString();
        if (this.minions[id] && this.minions[id].ws) this.minions[id].sendCaptcha(token);
    }
    sendMinionsCount(spawnedMinions, minionsAmount) {
        const writer = new Writer(5);
        writer.writeUint8(2);
        writer.writeUint16(spawnedMinions); 
        writer.writeUint16(minionsAmount);
        this.ws.send(writer.buffer);
    }
    updateAliveMinions() {
        const maxMinions = config.minionsAmount;
        this.aliveMinions = this.minions.filter(minion => minion.ws?.readyState === WebSocket.OPEN && minion.isAlive).length;
        if (this.aliveMinions > maxMinions) this.aliveMinions = maxMinions;
        this.sendMinionsCount(this.aliveMinions, maxMinions);
    }
}
...
class NELBOTS {
	...
	spawn() {
		const string = config.minionsName + "|" + Helper.randomString(10);
